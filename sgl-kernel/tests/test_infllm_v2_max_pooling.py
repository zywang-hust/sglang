import pytest
import torch
from sgl_kernel import max_pooling_1d, max_pooling_1d_varlen
...
@pytest.mark.parametrize("dtype", [torch.float16, torch.bfloat16])
def test_max_pooling_fixed_runs(dtype):
    num_heads, q_len, k_len = 2, 40, 32
    score = torch.randn(num_heads, q_len, k_len, dtype=dtype, device="cuda")
    out = max_pooling_1d(score, cache_len=0, local_blocks=1, init_blocks=1)
    block_size = 64
    expected_out_len = (q_len + block_size - 1) // block_size
    assert out.shape == (num_heads, q_len, expected_out_len)
