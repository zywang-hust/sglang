...
void run_mha_bwd(Flash_bwd_params& params, cudaStream_t stream) {
  FP16_SWITCH(!params.is_bf16, [&] {
    HEADDIM_SWITCH(params.d, [&] {
      BOOL_SWITCH(params.is_causal, Is_causal, [&] { run_mha_bwd_<elem_type, kHeadDim, Is_causal>(params, stream); });
    });
  });
}
...
std::vector<at::Tensor> mha_bwd(
  ...
  if (head_size > 192 && is_dropout) {
    TORCH_CHECK(
        is_sm80 || is_sm90, "FlashAttention backward for head dim > 192 with dropout requires A100/A800 or H100/H800");
  }
  TORCH_CHECK(num_heads % num_heads_k == 0, "Number of heads in key/value must divide number of heads in query");
  ...
  if (loop) {
    if (!deterministic) {
      ...
      dq_accum =
          torch::zeros({nsplits, batch_size, seqlen_q_rounded, num_heads, head_size_rounded}, opts.dtype(at::kFloat));
    }
...
std::vector<at::Tensor> mha_varlen_bwd(
  ...
  if (loop) {
    ...
    if (!deterministic) {
      ...
      const int nsplits = (get_num_sm(get_current_device()) + batch_size * num_heads - 1) / (batch_size * num_heads);
      dq_accum =
          torch::zeros({nsplits, total_q + 128 * batch_size, num_heads, head_size_rounded}, opts.dtype(at::kFloat));
    }
  }
...
std::vector<at::Tensor> mha_fwd_kvcache(
  ...
  // or paged KV cache
  run_mha_fwd(
      params, stream, /*force_split_kernel=*/k_.has_value() || cache_batch_idx_.has_value() || paged_KV || true);
