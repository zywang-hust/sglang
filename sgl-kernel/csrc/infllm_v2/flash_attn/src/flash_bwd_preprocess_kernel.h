...
inline __device__ void dot_do_o(
  ...
  Tensor do_reshaped = make_tensor(
      do_.data(), make_layout(get<1>(do_.layout()), make_layout(get<0>(do_.layout()), get<2>(do_.layout()))));
...
inline __device__ void clear_dKVaccum(const Params& params) {
  ...
  const index_t row_offset_dkv_accum =
      ((bidb * params.h_k + bidh) * params.seqlen_k_rounded + n_block * kBlockN) * params.d_rounded;
...
inline __device__ void convert_dKV(const Params& params) {
  ...
  Tensor sdK = make_tensor(make_smem_ptr(reinterpret_cast<Element*>(smem_)), typename Kernel_traits::SmemLayoutdKV{});
