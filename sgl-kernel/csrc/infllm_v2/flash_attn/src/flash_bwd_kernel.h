...
make_tiled_copy_B_warpcontiguousN(Copy_Atom<Args...> const& copy_atom, TiledMMA const& tiled_mma) {
...
template <int MMA_N, class... Args, class TiledMMA>
CUTE_HOST_DEVICE auto
make_tiled_copy_C_warpcontiguousN(Copy_Atom<Args...> const& copy_atom, TiledMMA const& tiled_mma) {
...
compute_dq_dk_dv_1colblock(const Params& params, const int bidb, const int bidh, const int n_block) {
  ...
  constexpr int MMA_N_SdP =
      kBlockN / decltype(typename Kernel_traits::TiledMmaSdP{}.template tile_size_mnk<1>())::value;
  ...
  if (Is_local) {
    m_block_max = std::min(
        m_block_max,
        cute::ceil_div(
            (n_block + 1) * kBlockN + binfo.actual_seqlen_q - binfo.actual_seqlen_k + params.window_size_left,
            kBlockM));
  }
  ...
  const index_t row_offset_dq_accum =
      ...
      ((m_block_max - 1) * kBlockM + (params.cu_seqlens_q == nullptr ? 0 : 128 * bidb)) * params.h * params.d_rounded +
      bidh * params.d_rounded
      // If deterministic, each thread block will do atomicAdd to a different dQ_accum buffer.
      + (!params.deterministic ? 0 : blockIdx.x * params.dq_accum_split_stride);
  const index_t row_offset_lse = (params.unpadded_lse ? bidh * params.total_q + binfo.q_offset(params.seqlen_q, 1, bidb)
  ...
  // auto smem_thr_copy_KV = make_tiled_copy_B(typename Kernel_traits::SmemCopyAtom{},
  // tiled_mma_sdp).get_thread_slice(tidx);
  auto smem_tiled_copy_KV =
      make_tiled_copy_B_warpcontiguousN<MMA_N_SdP>(typename Kernel_traits::SmemCopyAtom{}, tiled_mma_sdp);
  ...
  // auto smem_thr_copy_PdS = make_tiled_copy_C(typename Kernel_traits::SmemCopyAtomPdS{},
  // tiled_mma_sdp).get_thread_slice(tidx);
  auto smem_tiled_copy_PdS =
      make_tiled_copy_C_warpcontiguousN<MMA_N_SdP>(typename Kernel_traits::SmemCopyAtomPdS{}, tiled_mma_sdp);
...
#pragma unroll
  ...
  int m_block_min = (!Is_causal && !Is_local) ? 0
                                              : std::max(
                                                    0,
                                                    (n_block * kBlockN + (binfo.actual_seqlen_q / params.m_block_dim) -
  ...
  // Recall that m_block_max = cute::ceil_div(binfo.actual_seqlen_q, kBlockM) = (actual_seqlen_q + kBlockM - 1) /
  // kBlockM. So m_block_m - 1 = (actual_seqlen_q - 1) / kBlockM. We conclude that m_block_min <= m_block, so we will
  // always have at least 1 iteration of the for loop. However, if local, then this possible to have some blocks of K &
  // V not attending to any query. We might need to exit early and write 0 to dK and dV for those blocks. Otherwise we
  // get wrong result for the case where we don't enter the for loop. And we might read OOB elements from gQ and gdO.
...
#pragma unroll
  ...
  flash::Dropout dropout(
      params.rng_state[0], params.rng_state[1], params.p_dropout_in_uint8_t, bidb, bidh, tidx, params.h);
...
#pragma unroll
    ...
    if (!Is_causal && !Is_local) {
      if (!Is_even_MN && (n_block + 1) * kBlockN >= binfo.actual_seqlen_k) {
        flash::apply_mask(
            scores, binfo.actual_seqlen_k, n_block * kBlockN + (tidx / 32 / AtomLayoutMS) * MMA_N_SdP * 16);
      }
    } else if (Is_causal) {
      ...
      if (m_block * kBlockM <
              (n_block + 1) * kBlockN + binfo.actual_seqlen_q - binfo.actual_seqlen_k - params.window_size_right ||
          (m_block + 1) * kBlockM >=
    ...
    Tensor rP = !Is_dropout ? flash::convert_type<Element>(acc_s) : flash::convert_type_relu<Element>(acc_s);
...
#pragma unroll
    ...
    //     m_block, n_block, next_block_row_idx, next_leap);
    // }
  }
...
inline __device__ void compute_dq_dk_dv(const Params& params) {
  ...
  if (n_block_max == 1) {
    compute_dq_dk_dv_1colblock<Kernel_traits, Is_dropout, Is_causal, Has_alibi, Is_even_M, Is_even_K, true, true>(
        params, bidb, bidh, 0);
  } else {
    // Iterating backward from n_block_max - 1 to 0 might save 1 register
    compute_dq_dk_dv_1colblock<Kernel_traits, Is_dropout, Is_causal, Has_alibi, Is_even_M, Is_even_K, true, false>(
        params, bidb, bidh, n_block_max - 1);
    for (int n_block = n_block_max - 2; n_block > 0; n_block--) {
      compute_dq_dk_dv_1colblock<Kernel_traits, Is_dropout, Is_causal, Has_alibi, Is_even_M, Is_even_K, false, false>(
          params, bidb, bidh, n_block);
    }
    compute_dq_dk_dv_1colblock<Kernel_traits, Is_dropout, Is_causal, Has_alibi, Is_even_M, Is_even_K, false, true>(
        params, bidb, bidh, 0);
  }
}
...
inline __device__ void compute_dq_dk_dv_seqk_parallel(const Params& params) {
  ...
  // If deterministic, each thread block will do atomicAdd to a different dQ_accum buffer.
  for (int n_block = blockIdx.x; n_block < (params.seqlen_k + Kernel_traits::kBlockN - 1) / Kernel_traits::kBlockN;
       n_block += gridDim.x) {
