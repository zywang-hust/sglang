...
def _load_tree_links(
    ...
    return token_indices, next_tokens, next_siblings
...
def _advance_tree_parent(
    ...
    if i != 0:
        # parent < i, so the parent snapshot is already written; the barrier
        # publishes the other threads' stores before this load reads them.
        tl.debug_barrier()
...
# retrieve_next_token and retrieve_next_sibling come as a pair or not at all.
@triton.jit
def seg_la_mtp_kernel(
    ...
    if HAS_TREE:
        token_indices, next_tokens, next_siblings = _load_tree_links(
        ...
        # Unreferenced tokens default to parent 0, so the all-zero capture buffers
        # still load an in-bounds snapshot; token 0 never reads its entry.
    ...
    if MAX_LENGTH > 1:
        ...
        if caches is not None:
            ...
            if HAS_TREE:
                ...
                stride_retrieve_next_sibling_seq = 0
                stride_retrieve_next_sibling_token = 0
