"""Fused per-head q/k rmsnorm and NeoX rope, in place on a fused qkv tensor.

The generic fused_qk_norm_rope op recomputes cos/sin in kernel under
fast-math; this kernel reads the fp32 cos_sin_cache the eager rope reads
and rounds the normed value to the storage dtype before rotation, as the
eager chain does. Only the fp32 variance reduction order differs from the
eager rmsnorm kernel. qkv must be contiguous: the kernel derives the row
stride as NH_QKV * D."""
...
# 16 rows per program and 2 warps per 128-wide head; not tuned.
_BLOCK_M = 16
_NUM_WARPS = 2
...
def _minicpm_qknorm_rope_kernel(
    ...
    # Same rounding point as the eager rmsnorm kernel to rope kernel chain.
...
def minicpm_qknorm_rope(
    ...
    """Normalize and rotate the q/k segments of ``qkv`` in place and return
    q/k/v as ``(num_tokens, heads, head_dim)`` views into it; cos_sin_cache is
    ``(max_position, head_dim)`` fp32 with ``[cos | sin]`` per row."""
    num_tokens = qkv.shape[0]
    num_qkv_heads = num_q_heads + num_k_heads + num_v_heads
    grid = (triton.cdiv(num_tokens, _BLOCK_M), num_q_heads + num_k_heads)
    ...
    heads = qkv.view(num_tokens, num_qkv_heads, head_dim)
    return (
        heads[:, :num_q_heads],
