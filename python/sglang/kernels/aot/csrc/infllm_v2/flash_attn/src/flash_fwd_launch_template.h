...
void run_mha_fwd_splitkv_dispatch(Flash_fwd_params& params, cudaStream_t stream) {
  if (params.blockmask == nullptr) {
    ...
    if (params.m_block_dim == 1) {
      ...
      // Causal prefill packs params.m_block_dim (16) query rows per token, so kBlockM=64 reuses each
      // K-tile across four query tokens; non-causal decode has only 16 rows per sequence, which would
      // idle three of four warps.
