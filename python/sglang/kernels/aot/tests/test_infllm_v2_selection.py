...
def _ref_stage1(q, k1, k2, query_len):
    """Independent einsum reference for the stage-1 score and normalization
    path, the part a retiled kernel can break at partial 64-row tiles. The
    mask limits mirror ``apply_mask_stage1`` (the key frontier advances one
    key per 16 (k1) / 64 (k2) query tokens, anchored so the last query token
    sees every key) and are not validated here."""
...
def test_varlen_segments_match_single_runs():
    ...
    def run(q, k1, k2, cu_rows, cu_k1, cu_k2):
        return sgl.infllmv2_attn_stage1(
            ...
            cu_seqlens_k=cu_k1,
            cu_seqlens_v=cu_k2,
            max_seqlen_q=max(segment[0].shape[0] for segment in segments) * groups,
            max_seqlen_k=k1_len,
            causal=True,
        )
