...
def max_pooling_1d_varlen(
    ...
    Input is a stage-1 score buffer (num_heads x total_q x max_k):
    the key axis is allocated rounded up to a multiple of 128,
    so the row stride is the buffer's actual width (``input.shape[2]``),
    not ``max_context_len // stride``.
    """
