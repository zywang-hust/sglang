...
class MiniCPMSparseMetadata(msgspec.Struct):
    ...
    plus the row, selection and compression fields the MiniCPM backend plans around it.
    A msgspec.Struct (not the dataclass metadata family of the mamba backends)
    so that assigning an undeclared field raises instead of silently adding it."""

    # FlashAttention base geometry shared with the wrapped FA backend.
    ...
    # Dense decode rows as (row_start, batch_idx, kv_len),
    # with row_start = batch_idx * head_group_num.
...
def _assign_row_metadata(
    ...
    *,
    ...
    """Assign the row-shaped selection metadata for eager decode rows. No
    field has a default, so the call site must pass the complete set.
    ``seqlen_k_sparse_bs_tensor`` is always the base cache lengths and is
    read off ``metadata.base`` here."""
...
class SparseDecodeMetadata(msgspec.Struct):
    """Row-shaped decode metadata: one single-query row per (token, head group),
    with the dense-row overwrite list the page-table copy reads."""
