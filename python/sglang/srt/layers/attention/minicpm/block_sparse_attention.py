"""Block-native sparse prefill attention for MiniCPM InfLLM block selection.

Each query token carries its own top-k KV block list;
a query tile unions those lists and keeps a per-query bitmask on every union entry,
so one loaded K/V block serves the tile,
while each row applies its own blocks and causal frontier.
"""
...
# Query-tile width cap. Wider tiles dilute sparsity;
# 8 also keeps QUERY_TILE * HEADS_PER_GROUP within _QUERY_ROWS,
# given the 16 query heads per KV head.
_MAX_QUERY_TILE = 8
...
def block_sparse_attention(
    *,
    ...
    """Sparse prefill for one contiguous request;
    tiles whose union exceeds union_threshold fall back to the pair kernel.
    Requires seq_len == prefix_len + num_tokens: the causal bound then implies
    the seq_len bound, so only the KV and page_table loads mask on seq_len."""
    num_tokens, num_q_heads, head_dim = q.shape
    num_kv_heads = k_cache.shape[1]
    heads_per_group = num_q_heads // num_kv_heads
    query_tile = min(_MAX_QUERY_TILE, _QUERY_ROWS // heads_per_group)
    num_tiles = triton.cdiv(num_tokens, query_tile)
    num_blocks = triton.cdiv(seq_len, block_size)
    topk = topk_idx.shape[-1]
    # Tile kernel wastes (union - topk) block scores per query;
    # the pair kernel's waste is bounded by topk (union <= 2 * topk).
    # union_threshold = topk + 2 * topk // 3 is the tuned crossover,
    # balancing shared-union dilution against the pair kernel's worst case.
