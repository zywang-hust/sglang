...
# TL_DISABLE_THREAD_STORAGE_SYNC must stay off:
# the per-round shared-memory bitonic sort relies on the auto-inserted __syncthreads();
# without that sync it races on any shape, single-round included.
_pass_configs = {
...
def _fused_attn_pooling_online_topk(
    ...
    def main(
        ...
        with T.Kernel(max_seqlen_q_grid, head_kv, batch_size, threads=threads) as (
            ...
            # Candidates are the token's causally visible blocks 0..own;
            # the local-block boost selects the token's own block,
            # even before its compressed row exists.
