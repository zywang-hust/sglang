...
class TestVerifyGraphReplay(CustomTestCase):
    def _assert_replay_rebuilds_base_geometry(
        ...
        """Base geometry after replay: prefix + draft lengths, cumsum KV,
        and page_table rows matching req_to_token."""
    ...
    def _assert_level_buffers_match_closed_form(
        ...
        """K1/K2 level buffers against the closed-form reference,
        for the padded batch."""
        seq_tensor = torch.tensor(real_seq_lens, dtype=torch.int32, device=DEVICE)
        ...
        for name, kernel_size, kernel_stride in (
            ...
            self.assertTrue(
                (level.cu_seqlens[real_bs + 1 :] == expected_cu[-1]).all(), name
            )
            expected_history = torch.clamp(
                (seq_tensor - kernel_size) // kernel_stride + 1, min=0
            )
            self.assertTrue(
                torch.equal(
                    level.history_compress_token_nums[:real_bs], expected_history
                ),
                name,
            )
            self.assertTrue(
            ...
            expected_new = (
                seq_tensor + num_draft_tokens - expected_history * kernel_stride
            )
            expected_cu_new = F.pad(
                torch.cumsum(expected_new, dim=0, dtype=torch.int32), (1, 0)
            )
            self.assertTrue(
                torch.equal(level.cu_new_token_nums[: real_bs + 1], expected_cu_new),
                name,
            )
            self.assertTrue(
                (level.cu_new_token_nums[real_bs + 1 :] == expected_cu_new[-1]).all(),
                name,
            )
    ...
    def test_replay_refreshes_static_buffers_to_eager_geometry(self):
        ...
        backend, base = make_replay_backend(
            padded_seq_lens, num_draft_tokens, max_bs=4, device=DEVICE
        )
        head_group_num = backend.head_group_num
        ...
        forward_batch, metadata = _replay_verify_graph(
            ...
            req_pool_indices=torch.tensor([2, 5, 1, 0], device=DEVICE),
            spec_info=None,
        )
        ...
        # K1/K2 level buffers against the closed-form reference,
        # for the padded batch.
        self._assert_level_buffers_match_closed_form(
    ...
    def test_graph_tree_replay_matches_eager_tree_builder(self):
        ...
        bs, real_bs = len(padded_seq_lens), len(real_seq_lens)
        backend, base = make_replay_backend(
            padded_seq_lens, num_draft_tokens, max_bs=3, eagle_topk=2, device=DEVICE
        )
        head_group_num = backend.head_group_num
        ...
        forward_batch, metadata = _replay_verify_graph(
            ...
            real_bs=real_bs,
            padded_seq_lens=padded_seq_lens,
            req_pool_indices=torch.tensor([2, 5, 1], device=DEVICE),
        ...
        expected_history = torch.clamp(
            (
                torch.tensor(real_seq_lens, dtype=torch.int32, device=DEVICE)
                - num_draft_tokens
                - kernel_size
            )
            // kernel_stride
...
class TestCompressLevelMetadataKernel(CustomTestCase):
    def test_bit_exact_against_host_builder(self):
        """Pin fill_compress_level_metadata bitwise to the host floor-div+clamp oracle,
        ...
        for prefix_delta in (0, -num_draft_tokens):
            ...
            full = seq[:real_bs].cpu() + num_draft_tokens
            boundary = (seq[:real_bs].cpu() + prefix_delta).clamp(min=0)
            want_lens = torch.clamp((full - kernel_size) // kernel_stride + 1, min=0)
            want_history = torch.clamp(
                (boundary - kernel_size) // kernel_stride + 1, min=0
            )
            want_new = full - want_history * kernel_stride
            self.assertEqual(
                history[:real_bs].cpu().tolist(), want_history.tolist(), prefix_delta
            )
            self.assertTrue((history[real_bs:] == 0).all())
            want_cu = F.pad(torch.cumsum(want_lens, dim=0, dtype=torch.int32), (1, 0))
            self.assertEqual(
                cu_seqlens[: real_bs + 1].cpu().tolist(),
                want_cu.tolist(),
                prefix_delta,
            )
            self.assertTrue((cu_seqlens[real_bs + 1 :].cpu() == want_cu[-1]).all())
            want_cu_new = F.pad(
                torch.cumsum(want_new, dim=0, dtype=torch.int32), (1, 0)
            )
            self.assertEqual(
                cu_new[: real_bs + 1].cpu().tolist(),
                want_cu_new.tolist(),
                prefix_delta,
            )
            self.assertTrue((cu_new[real_bs + 1 :].cpu() == want_cu_new[-1]).all())
