...
class TestMiniCPMRopeRoundTrip(CustomTestCase):
    ...
    def test_bf16_rope_matches_fp32_roundtrip(self):
        ...
        q = (torch.randn((T, nh * d), generator=gen) * 0.4).to(DEVICE, torch.bfloat16)
        k = (torch.randn((T, nh * d), generator=gen) * 0.4).to(DEVICE, torch.bfloat16)
        positions = torch.randint(0, 4096, (T,), generator=gen).to(DEVICE)
        rope = get_rope(d, rotary_dim=d, max_position=8192, base=10000).to(DEVICE)

        q_rt, k_rt = rope(positions, q.clone().float(), k.clone().float())
