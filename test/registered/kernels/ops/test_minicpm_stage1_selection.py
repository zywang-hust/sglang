...
def test_selection_emits_topk_blocks_in_index_order():
    """Selection must emit each row's top-k blocks in ascending block-id order.

    The emitted set is the top-k by pooled score and each row is re-sorted by block id,
    so topk's internal value ordering is unobservable in the output:
    a block with a maximal pooled score must appear in every row,
    and the emitted indices must come out strictly ascending as int32.
    """
    query_len = 17
    context_len = 8192
    max_score_block = 20
